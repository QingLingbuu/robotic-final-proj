...
## 2026-05-10 RL Cup/Mug Ordering Minimal PPO Training Loop

- Completed: added a minimal discrete PPO training environment `CupOrderingTrainEnv` in `rl/harness.py`. It wraps the existing 5-slot cup-ordering observation, action mask, and `CupMugOrderingEpisodeRunner` into a Gym-compatible train loop without changing the dry-run evaluation path.
- Completed: `configs/rl/cup_mug_ordering_robocasa.yaml` now includes a `train` section for cup-ordering (`smoke_steps`, `total_timesteps`, `n_steps`, `progress_print_freq`, `allowed_failed_attempts`) so train/eval entrypoints share one explicit config contract.
- Completed: `run_train_smoke()` for `mode: cup_ordering` now performs real PPO training when `--dry-run` is not set, saves a real `.zip` checkpoint, and writes a compact training summary JSON under `outputs/rl_cup_mug_ordering/metrics/`.
- Completed: checkpoint inference compatibility is preserved. The existing `select_cup_ordering_rl_action()` `.zip` loading path can now consume a checkpoint produced by cup-ordering training instead of only the stub or dry-run JSON artifact.
- Completed: extended `tests/test_rl_contracts.py` with a fake-PPO regression that verifies the cup-ordering branch instantiates `CupOrderingTrainEnv`, calls `learn()`, saves a `.zip`, and writes the training summary artifact.
- Completed: `ProgressCallback` now records train-time rollout and loss metrics (`rollout/ep_rew_mean`, `train/loss`, `train/value_loss`, `train/policy_gradient_loss`, etc.) and writes them to `outputs/rl_cup_mug_ordering/metrics/cup_ordering-train-history-<timesteps>.json` after PPO training.
- Completed: added `scripts/plot_cup_mug_ordering_report.py` to generate English report-ready figures from evaluation JSON and training-history JSON. Current figure set includes completion-count histogram, cumulative mean completion ratio, cumulative full-episode success rate, RL training reward curve, and RL training loss curves.
- Completed: identified why `random` / `greedy` / `rl` dry-run report figures initially collapsed to `5/5` for every episode. The cause was not the state machine; the dry-run evaluator was still wired to a placeholder executor that always returned `low_level_success=True` and `correct_zone=True`.
- Completed: updated cup-ordering dry-run evaluation to reuse the same stochastic success / failure simulator as the minimal PPO training environment, so baseline comparison figures now reflect policy-dependent ordering quality instead of a guaranteed-success executor.
- Completed: rewrote the root `README.md` as a teacher-facing application workflow document. It now focuses on environment use, online demo commands, RL training, real RoboCasa batch evaluation, and figure generation instead of internal development conventions.
- Completed: added a first-pass real RoboCasa batch evaluator `scripts/eval_rl_real_cup_mug_ordering.py` that reuses the live online execution chain, supports `random` / `greedy` / `rl` policies, and writes metrics JSON compatible with the existing report plotting script.
- Verification: pending in this cycle. Recommended commands are `python -m unittest discover -s tests -p "test_rl_contracts.py"` and then `python scripts/train_rl.py --config configs/rl/cup_mug_ordering_robocasa.yaml --timesteps 256` followed by `python scripts/eval_rl.py --config configs/rl/cup_mug_ordering_robocasa.yaml --policy rl --checkpoint outputs/rl_cup_mug_ordering/checkpoints/cup-ordering-ppo-256.zip --episodes 5 --dry-run`.
- Recommended next step: owner group `eval/logging`; goal: rerun the 50-episode `random` / `greedy` / `rl` dry-run evaluations from the same fixed checkpoint after the stochastic dry-run executor change, then regenerate the report figures with `scripts/plot_cup_mug_ordering_report.py`; success criteria: completion histograms are no longer degenerate at `5/5` for every policy, and the cumulative curves now show meaningful differences across `random`, `greedy`, and `rl`.
